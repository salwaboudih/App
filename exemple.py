...
# Charger le modèle avec gzip
with gzip.open('besthealthcare1.pkl.gz', 'rb') as f:
    model = pickle.load(f)

st.title("Prédiction de Risque d'AVC")

# Demander les inputs à l'utilisateur
...
if st.button("Prédire le risque d'AVC"):
    ...
    # Convertir les choix utilisateur en valeurs numériques
    ...
    # Créer un tableau numpy avec les données utilisateur
    input_data = np.array([[
        gender,
        age,
        hypertension,
